...
(function(){
  ...
  function ensureMobileNav(){
    ...
    const menu=document.createElement('button'); menu.type='button'; menu.className='mf-mobile-menu'; menu.setAttribute('aria-label','Open navigation'); menu.textContent='☰';
    header.insertBefore(menu,header.firstChild);
    const close=()=>{document.body.classList.remove('mf-nav-open');menu.setAttribute('aria-expanded','false');};
    menu.onclick=()=>{const open=document.body.classList.toggle('mf-nav-open');menu.setAttribute('aria-expanded',String(open));};
    qsa('.sidebar .nav[data-view],#logout').forEach(el=>el.addEventListener('click',()=>{if(mobile())close();}));
    document.addEventListener('click',e=>{if(document.body.classList.contains('mf-nav-open')&&e.target===document.body)close();});
    addEventListener('resize',()=>{if(!mobile())close();},{passive:true});
  }
  function ensureFab(){
  ...
  ensureMobileNav();ensureFab();legalLinks();onboarding();
})();
