...
(function(){
  ...
  const companyName=()=>$('.store-card b')?.textContent?.trim()||business().name||'Your business';
  const businessDetail=()=>{
    const b=business();
    const parts=[b.phone,b.address].filter(Boolean);
    return parts.join(' · ');
  };
  ...
  function renderVeloraInvoice(){
    ...
    const rawCustomer=$('#customerName')?.value?.trim()||'';
    const rawPhone=$('#phone')?.value?.trim()||'';
    const customer=rawCustomer||'Enter customer name';
    const phone=rawPhone||'Add mobile number';
    ...
    preview.innerHTML=`
      <div class="velora-receipt-head">
        <div>
          ...
          <div class="velora-powered">Business invoice${detail?` · ${escHtml(detail)}`:''}</div>
        </div>
      ...
      <div class="velora-items">
        ...
        <div class="velora-line muted-line"><span>Subtotal</span><span>${fmt(subtotal)}</span></div>
        <div class="velora-line muted-line"><span>Discount</span><span>− ${fmt(discount)}</span></div>
      </div>
      <div class="velora-total"><div><span>TOTAL PAYABLE</span><small>Final amount after discount</small></div><strong>${fmt(total)}</strong></div>
      <div class="velora-receipt-foot"><div><strong>Thank you for your business.</strong><small>We appreciate your purchase.</small></div><b>Velora</b></div>`;
  }
  ...
  window.printInvoice=function(id){
    const i=findInvoice(id);if(!i)return;
    const company=companyName(),detail=businessDetail(),w=open('','_blank','width=780,height=980');if(!w)return;
    w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${escHtml(i.id)}</title><link rel="preconnect" href="https://fonts.googleapis.com"><link href="https://fonts.googleapis.com/css2?family=DM+Serif+Display&family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap" rel="stylesheet"><style>*{box-sizing:border-box}body{margin:0;background:#eee7d7;color:#182019;font-family:'Plus Jakarta Sans',sans-serif;padding:36px}.sheet{max-width:700px;margin:auto;background:#fffdf8;border-radius:28px;overflow:hidden;border:1px solid #d8cfbd;box-shadow:0 24px 70px rgba(24,32,25,.12)}.top{background:linear-gradient(135deg,#153d32,#1f5b49 62%,#2a7660);color:#fff;padding:34px 38px;display:flex;justify-content:space-between;gap:24px}.company{font:400 34px 'DM Serif Display',serif}.sub{margin-top:7px;color:#dce9e2;font-size:12px}.brand{text-align:right}.brand b{display:block;font:400 31px 'DM Serif Display',serif}.brand small{font-size:9px;letter-spacing:.18em;color:#cfe1d8}.body{padding:34px 38px}.meta{display:grid;grid-template-columns:repeat(2,1fr);gap:18px;background:#f4f0e6;border-radius:18px;padding:20px;margin-bottom:24px}.label{font-size:9px;letter-spacing:.14em;font-weight:800;color:#667069}.value{font-weight:700;margin-top:5px}.rows{border-top:1px solid #ded6c8}.row{display:flex;justify-content:space-between;gap:20px;padding:17px 0;border-bottom:1px solid #e7e0d4}.row small{display:block;color:#667069;margin-top:4px}.total{margin-top:24px;background:#1f5b49;color:#fff;border-radius:18px;padding:22px 24px;display:flex;align-items:end;justify-content:space-between;gap:20px}.total span{font-size:10px;letter-spacing:.13em;font-weight:800;color:#dce9e2}.total strong{font:400 34px 'DM Serif Display',serif}.foot{margin-top:24px;background:#f4f0e6;border-radius:16px;padding:18px 20px;display:flex;justify-content:space-between;gap:20px;align-items:center;color:#667069;font-size:11px}.foot strong{display:block;color:#182019;font-size:14px;margin-bottom:4px}.foot b{font:400 22px 'DM Serif Display',serif;color:#1f5b49}@media print{body{background:#fff;padding:0}.sheet{box-shadow:none;border:0;border-radius:0;max-width:none}}</style></head><body><article class="sheet"><div class="top"><div><div class="company">${escHtml(company)}</div><div class="sub">Business invoice${detail?` · ${escHtml(detail)}`:''}</div></div><div class="brand"><b>Velora</b><small>BUSINESS, MADE CLEAR.</small></div></div><div class="body"><div class="meta"><div><div class="label">INVOICE</div><div class="value">${escHtml(i.id)}</div></div><div><div class="label">DATE</div><div class="value">${escHtml(i.date)}</div></div><div><div class="label">CUSTOMER</div><div class="value">${escHtml(i.customer||'Customer')}</div></div><div><div class="label">MOBILE</div><div class="value">${escHtml(i.phone||'—')}</div></div></div><div class="rows"><div class="row"><span><b>${escHtml(i.product)}</b><small>${i.qty} × ${fmt(i.rate)}</small></span><strong>${fmt(i.subtotal||i.qty*i.rate)}</strong></div><div class="row"><span>Subtotal</span><span>${fmt(i.subtotal||i.qty*i.rate)}</span></div><div class="row"><span>Discount</span><span>− ${fmt(i.discount||0)}</span></div><div class="row"><span>Payment</span><span>${escHtml((i.paymentMethod||'upi').toUpperCase())} · ${escHtml(i.paymentStatus||'paid')}</span></div></div><div class="total"><div><span>TOTAL PAYABLE</span></div><strong>${fmt(i.total)}</strong></div><div class="foot"><span><strong>Thank you for your business.</strong>We appreciate your purchase.</span><b>Velora</b></div></div></article><script>window.onload=()=>window.print()<\/script></body></html>`);w.document.close();
  };
