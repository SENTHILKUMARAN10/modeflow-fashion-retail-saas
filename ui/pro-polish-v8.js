...
(function(){
  ...
  function polishPreview(){
    ...
    const head=preview.querySelector('.bill-head');if(!head)return;
    const left=head.children?.[0],right=head.children?.[1];
    const company=businessName();
    if(left){const b=left.querySelector('b');if(b&&b.textContent!==company)b.textContent=company;const m=left.querySelector('.muted');if(m)m.textContent='Powered by Velora';}
    if(right){const b=right.querySelector('b');if(b)b.textContent='VELORA RECEIPT';const m=right.querySelector('.muted');if(m)m.textContent='Business, made clear.';}
  }
  ...
  function findInvoice(id){return window.store?.invoices?.find?.(x=>x.id===id)||null;}
  window.shareInvoice=function(id){
    ...
    const url=`https://wa.me/${phone}?text=${encodeURIComponent(invoiceText(i))}`;
    window.open(url,'_blank','noopener');
  };
  window.printInvoice=function(id){
    const i=findInvoice(id);if(!i)return;
    const company=businessName();
    const w=open('','_blank','width=760,height=940');if(!w)return;
  ...
  function bind(){
    resolveClientName();updateGreeting();cleanAdminDuplicates();polishPreview();
    ...
    addEventListener('modeflow:workspace',()=>{setTimeout(()=>{resolveClientName();polishPreview();cleanAdminDuplicates()},60)});
    const obs=new MutationObserver(()=>{cleanAdminDuplicates();polishPreview();updateGreeting()});
