// Velora final v18 — single authoritative invoice renderer; no competing receipt writers.
(function(){
  'use strict';
  ...
  function businessName(){
    ...
    for(const name of [visible,cloud]){
  ...
  function totals(){const qty=Math.max(1,Number($('#qty')?.value)||1),rate=Number($('#rate')?.value)||0,discount=Math.max(0,Number($('#discount')?.value)||0),subtotal=qty*rate,total=Math.max(0,subtotal-discount);return{qty,rate,discount,subtotal,total};}
  ...
  function renderReceipt(){
    ...
    try{
      ...
      preview.dataset.veloraFinal='18';
      preview.innerHTML=`<article class="vf-invoice">
        ...
        <section class="vf-items"><div class="vf-head"><span>ITEM</span><span>QTY</span><span>RATE</span><span>AMOUNT</span></div><div class="vf-row"><strong>${esc(productName())}</strong><span>${t.qty}</span><span>${money(t.rate)}</span><strong>${money(t.subtotal)}</strong></div></section>
        <section class="vf-summary"><div><span>Subtotal</span><strong>${money(t.subtotal)}</strong></div><div><span>Discount</span><strong>− ${money(t.discount)}</strong></div><div class="vf-total"><span>Total payable</span><strong>${money(t.total)}</strong></div></section>
  ...
  function syncProductRate(){
    const option=$('#product option:checked');
    const rate=$('#rate');
    if(!option||!rate)return;
    const text=option.textContent||'';
    const match=text.match(/[₹$]\s*([\d,]+(?:\.\d+)?)/);
    if(match){const value=Number(match[1].replace(/,/g,''));if(Number.isFinite(value))rate.value=String(value);}
  }
  ...
  function blockLegacyInputWriters(e){
    if(!e.target?.closest?.('#invoiceForm'))return;
    if(e.type==='change'&&e.target.id==='product')syncProductRate();
  ...
  function bind(){
    ...
    // Capture before app.js/runtime legacy listeners and render once. This prevents visual flicker.
    document.addEventListener('input',blockLegacyInputWriters,true);
    document.addEventListener('change',blockLegacyInputWriters,true);

    // Re-render once after any full data refresh, in the same JS turn before the browser paints.
    const oldRenderAll=window.renderAll;
    if(typeof oldRenderAll==='function'&&!oldRenderAll.__veloraWrapped){
      const wrapped=function(){const result=oldRenderAll.apply(this,arguments);renderReceipt();return result;};
      wrapped.__veloraWrapped=true;
      window.renderAll=wrapped;
    }

    document.addEventListener('click',e=>{if(e.target.closest?.('[data-view="billing"],.goto-billing'))queueMicrotask(renderReceipt)},true);
    addEventListener('modeflow:workspace',()=>queueMicrotask(()=>{removeSidebarClient();resolveClient();renderReceipt();applyInvoiceActions();}));

    const preview=$('#preview');
    if(preview)new MutationObserver(()=>{
      if(rendering)return;
      const text=preview.textContent||'';
      if(preview.dataset.veloraFinal!=='18'||/Powered by ModeFlow|ModeFlow POS/i.test(text)||!preview.querySelector('.vf-invoice'))queueMicrotask(renderReceipt);
