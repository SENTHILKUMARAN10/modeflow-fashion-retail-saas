...
const detect=/SalesDesk|SALESDESK|salesdesk|ModeFlow|MODEFLOW|modeflow|Velora|VELORA|velora|\b(?:VL|SD)-\d/i;
function replaceText(value){return String(value??'').replace(/SalesDesk|SALESDESK|salesdesk|ModeFlow|MODEFLOW|modeflow|Velora|VELORA|velora/g,BRAND).replace(/\b(?:VL|SD)-(?=\d)/g,'SV-')}
function apply(){
  ...
  const walker=document.createTreeWalker(root,NodeFilter.SHOW_TEXT,{acceptNode(n){const p=n.parentElement;if(!p||/^(SCRIPT|STYLE|NOSCRIPT|TEXTAREA)$/i.test(p.tagName))return NodeFilter.FILTER_REJECT;return detect.test(n.nodeValue||'')?NodeFilter.FILTER_ACCEPT:NodeFilter.FILTER_REJECT;}});const nodes=[];while(walker.nextNode())nodes.push(walker.currentNode);nodes.forEach(n=>n.nodeValue=replaceText(n.nodeValue));
...
document.addEventListener('submit',()=>setTimeout(apply,40),true);
document.addEventListener('click',e=>{if(e.target.closest?.('.nav,.action-btn,#addProduct,#cancelProduct,[data-go],[data-view]'))setTimeout(apply,40);},true);
})();