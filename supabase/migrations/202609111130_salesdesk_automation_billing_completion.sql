...
-- ---------------------------------------------------------------------------
-- Billing lifecycle: provider state is authoritative.
-- ---------------------------------------------------------------------------
...
-- Intentionally no browser policies. Billing events are service-role only.

-- Only provider-confirmed active subscriptions (or a short past-due grace window)
-- unlock paid business mutations. An authenticated checkout alone is not access.
...
-- ---------------------------------------------------------------------------
-- Delivery ledger and automation scheduling.
-- ---------------------------------------------------------------------------
...
create table if not exists public.scheduled_reports(
  ...
  whatsapp_language text not null default 'en_US',
...
for select
-- placeholder removed by the DO block below
;
...
do $$ begin
  ...
  create policy "managers manage recurring expenses" on public.recurring_expense_schedules for all
    using(public.salesdesk_can(business_id,'automation.manage'))
    with check(public.salesdesk_can(business_id,'automation.manage') and created_by=auth.uid());
  ...
  create policy "managers read automation executions" on public.automation_executions for select
    using(public.salesdesk_can(business_id,'automation.manage'));
end $$;

-- The previous placeholder is removed before execution by keeping the migration valid.
...
begin
  ...
  if not public.has_active_subscription(s.business_id) then
    update public.recurring_expense_schedules set last_error='Active SalesDesk subscription required' where id=s.id;
  ...
  insert into public.automation_executions(business_id,source_type,source_id,run_key)
    values(s.business_id,'recurring_expense',s.id,v_key) on conflict(source_type,source_id,run_key) do nothing returning id into v_exec;
  ...
  begin
    insert into public.expenses(business_id,branch_id,category,amount,note,expense_date,created_by)
      values(s.business_id,s.branch_id,s.category,s.amount,coalesce(s.note,'Recurring expense: '||s.name),(p_run_at at time zone 'UTC')::date,s.created_by)
...
begin
  ...
  v_key:=to_char(s.next_run_at at time zone 'UTC','YYYYMMDDHH24MI');
  insert into public.automation_executions(business_id,source_type,source_id,run_key)
    values(s.business_id,'recurring_invoice',s.id,v_key) on conflict(source_type,source_id,run_key) do nothing returning id into v_exec;
  if v_exec is null then return jsonb_build_object('status','skipped','reason','already processed');end if;
  begin
    if jsonb_typeof(s.items)<>'array' or jsonb_array_length(s.items)=0 then raise exception 'Recurring invoice has no items';end if;
    ...
    for v_item in select value from jsonb_array_elements(s.items) order by value->>'product_id' loop
      ...
      if v_product.track_stock then
        ...
        if v_available<v_qty then raise exception 'Not enough stock for %',v_product.name;end if;
      end if;
    ...
    if v_customer is null then
      insert into public.customers(business_id,name,email,phone) values(s.business_id,s.customer_name,nullif(trim(s.customer_email),''),nullif(trim(s.customer_phone),'')) returning id into v_customer;
    end if;
    ...
    for v_item in select value from jsonb_array_elements(s.items) order by value->>'product_id' loop
      ...
      if v_product.track_stock then
        ...
        if v_wh is not null and not coalesce(v_wh_default,false) then
          update public.inventory_balances set quantity=quantity-v_qty,updated_at=now() where warehouse_id=v_wh and product_id=v_product.id;
        end if;
...
-- Service-only report snapshot used by the scheduled delivery worker.
