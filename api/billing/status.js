...
export default async function handler(req,res){
  ...
  try{
    ...
    const rows=await supabaseAdmin(`/rest/v1/business_subscriptions?business_id=eq.${encodeURIComponent(businessId)}&select=provider,provider_subscription_id,provider_plan_id,plan_interval,currency,amount,status,current_period_end,next_charge_at,cancel_at_period_end,last_payment_at,last_event_at,last_event_type,grace_until,failure_count,updated_at&limit=1`);
    const sub=rows?.[0]||null,currency=sub?.currency||'INR';
