...
| 13 | Sales returns | PARTIAL | Return from any transaction (Transactions → Return): select item/qty, refund method incl. credit note, reason, restock toggle; records sales_returns via create_sales_return RPC (stock + stock_movements). Add a returns register view |
