(function(){
  ...
  const state={
    region:localStorage.getItem('mf_region')||localStorage.getItem('modeflow-region')||'IN',
  ...
  if(!main)return;
  if(!$('#subscription')) main.insertAdjacentHTML('beforeend',`<section id="subscription" class="view"><div class="page-intro"><div><p class="kicker">MODEFLOW PRO</p><h3>Simple pricing, built to <em>grow with you.</em></h3><p class="mf-section-copy">Choose monthly or annual billing and complete payment securely using the configured QR payment flow.</p></div><span class="mf-settings-status">QR payment available</span></div><div class="mf-account-grid"><article class="mf-account-card featured"><p class="kicker light">PRO PLAN</p><h3>Everything your business needs.</h3><div class="mf-plan-switch"><button data-interval="monthly">Monthly</button><button data-interval="annual">Annual</button></div><div class="mf-plan-price" id="mfPlanPrice">₹399 <span>/ month</span></div><ul class="mf-feature-list"><li>Billing and sales management</li><li>Inventory and stock alerts</li><li>Customer records</li><li>Expenses and profit visibility</li><li>Analytics and cloud workspace</li><li>Responsive access on mobile, tablet and desktop</li></ul><div class="mf-billing-note">Tap the payment button to open the QR. After payment, enter the transaction/reference number so the payment can be verified.</div><div class="mf-account-actions"><button class="btn secondary" id="mfPayNow">Pay with QR</button></div></article><article class="mf-account-card"><p class="kicker">BILLING DETAILS</p><h3>Your current setup</h3><div class="mf-region-preview"><span>Region</span><strong id="mfBillingRegion">India</strong></div><div class="mf-region-preview"><span>Currency</span><strong id="mfBillingCurrency">INR · ₹</strong></div><div class="mf-region-preview"><span>Payment</span><span class="mf-settings-status">QR enabled</span></div><div class="mf-account-actions"><button class="btn primary" id="mfOpenRegion">Change region & currency</button></div></article></div></section>`);

  if(!$('#region')) main.insertAdjacentHTML('beforeend',`<section id="region" class="view"><div class="page-intro"><div><p class="kicker">PREFERENCES</p><h3>Your region. Your <em>currency.</em></h3><p>Choose how ModeFlow should display pricing and business values. You can change this anytime.</p></div></div><div class="mf-account-grid"><article class="mf-account-card"><div class="mf-region-form"><label>Business region<select id="mfRegionSelect">${Object.entries(regions).map(([v,l])=>`<option value="${v}">${l}</option>`).join('')}</select></label><label>Display currency<select id="mfCurrencySelect"><option value="INR">INR — Indian Rupee ₹</option><option value="USD">USD — US Dollar $</option></select></label><div class="mf-region-preview"><div><small>PREVIEW</small><strong id="mfCurrencyPreview">₹399</strong></div><span id="mfRegionPreview">India · INR</span></div><div class="mf-account-actions"><button class="btn primary" id="mfSaveRegion">Save preferences</button></div></div></article><article class="mf-account-card"><p class="kicker">HOW IT WORKS</p><h3>Consistent across every screen.</h3><p class="mf-section-copy">Your selected region and currency are saved for ModeFlow pricing and business values.</p><div class="mf-region-preview"><span>Monthly plan</span><strong id="mfMonthlyMini">₹399</strong></div><div class="mf-region-preview"><span>Annual plan</span><strong id="mfAnnualMini">₹3,990</strong></div></article></div></section>`);
  ...
  $('#mfPayNow')?.addEventListener('click',()=>{
    if(state.currency!=='INR'){
      if(typeof toast==='function')toast('QR payment is currently available for INR only.');
      else alert('QR payment is currently available for INR only.');
      return;
    }
    const open=()=>{
      const plans=$('#mfPlansButton');
      if(plans){plans.click();setTimeout(()=>{const pay=document.querySelector(`[data-mf-pay="${state.interval}"]`);if(pay)pay.click();},120);return true;}
      return false;
    };
    if(!open())setTimeout(()=>{if(!open()){const msg='Payment gateway is still loading. Please try again.';if(typeof toast==='function')toast(msg);else alert(msg);}},500);
  });
